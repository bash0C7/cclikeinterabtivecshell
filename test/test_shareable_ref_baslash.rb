...
class TestShareableRefBaslash < Test::Unit::TestCase
  class Counter
  ...
  def test_call_routes_method_to_owned_object_in_ractor
    ref = Baslash::ShareableRef.spawn(:counter) { Counter.new }
    ref.call(:add, 5)
    ref.call(:add, 7)
  ...
  def test_two_refs_have_isolated_state
    a = Baslash::ShareableRef.spawn(:a) { Counter.new }
    b = Baslash::ShareableRef.spawn(:b) { Counter.new }
  ...
  def test_stop_terminates_ractor
    ref = Baslash::ShareableRef.spawn(:c) { Counter.new }
    ref.stop
  ...
  def test_name_accessible
    ref = Baslash::ShareableRef.spawn(:my_name) { Counter.new }
