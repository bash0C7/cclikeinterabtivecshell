...
class TestSlashDispatcherBaslash < Test::Unit::TestCase
  ...
  def test_dispatch_slash_command_runs_body
    Baslash::SlashDispatcher.handle("/hello a b", @registry, on_submit: nil, logger: nil)
    assert_includes $stdout.string, "hi from hello"
  end

  def test_dispatch_unknown_command_prints_error
    Baslash::SlashDispatcher.handle("/unknown", @registry, on_submit: nil, logger: nil)
    assert_includes $stdout.string, "Unknown command: /unknown"
  end

  def test_dispatch_bare_slash_is_noop
    Baslash::SlashDispatcher.handle("/", @registry, on_submit: nil, logger: nil)
  ...
  def test_dispatch_non_slash_calls_on_submit
    ...
    Baslash::SlashDispatcher.handle("plain text", @registry, on_submit: on_submit, logger: nil)
  ...
  def test_dispatch_non_slash_without_on_submit_is_noop
    Baslash::SlashDispatcher.handle("plain text", @registry, on_submit: nil, logger: nil)
    assert_empty $stdout.string
  end

  def test_dispatch_rescues_handler_exceptions
    Baslash::SlashDispatcher.handle("/boom", @registry, on_submit: nil, logger: Baslash::Context.logger)
  ...
  def test_dispatch_rescues_interrupt
    ...
    Baslash::SlashDispatcher.handle("/slow", @registry, on_submit: nil, logger: nil)
  ...
  def test_dispatcher_invokes_handler_with_closure_capture
    ...
    Baslash::SlashDispatcher.handle("/closured hello", @registry, on_submit: nil, logger: nil)
