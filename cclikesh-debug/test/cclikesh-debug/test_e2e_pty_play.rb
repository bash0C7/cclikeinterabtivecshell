...
class TestPlayCli < Test::Unit::TestCase
  ...
  def test_no_alt_screen_spec_passes_under_play_cli
    ...
    Dir.chdir(repo_root) do
      ...
      begin
        code = Cclikesh::Debug::CLI::Play.call(argv: [spec, "--db", db], stdout: out)
        assert_equal 0, code, out.string
        assert_match(/^PASS: no \\e\[\?1049h \(smcup\) in captured output$/, out.string)
        assert_match(/^PASS: session exits cleanly$/, out.string)
      ensure
        [db, "#{db}-wal", "#{db}-shm"].each { |f| File.unlink(f) if File.exist?(f) }
      end
  ...
  def test_dividers_inline_spec_passes_under_play_cli
    repo_root = File.expand_path("../../..", __dir__)
    Dir.chdir(repo_root) do
      spec = File.join(repo_root, "cclikesh-debug/test/specs/dividers_inline.rb")
      out  = StringIO.new
      db   = File.join(Dir.tmpdir, "test-dividers-#{Process.pid}-#{rand(10000)}.sqlite")
      begin
        code = Cclikesh::Debug::CLI::Play.call(argv: [spec, "--db", db], stdout: out)
        assert_equal 0, code, out.string
        assert_match(/^PASS: captured output contains divider runs$/, out.string)
        assert_match(/^PASS: session exits cleanly$/, out.string)
      ensure
        [db, "#{db}-wal", "#{db}-shm"].each { |f| File.unlink(f) if File.exist?(f) }
      end
