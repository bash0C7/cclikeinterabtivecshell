...
class TestPlayCli < Test::Unit::TestCase
  ...
  def run_play(spec_path)
    out = StringIO.new
    code = Baslash::Debug::CLI::Play.call(
  ...
  def test_zsh_shell_slash_menu_spec_passes_under_play_cli
    ...
    Dir.chdir(repo_root) do
      spec = File.join(repo_root, "baslash-debug/test/specs/zsh_shell_slash_menu.rb")
      out  = StringIO.new
      db   = File.join(Dir.tmpdir, "test-zsh-#{Process.pid}-#{rand(10000)}.sqlite")
      begin
        code = Baslash::Debug::CLI::Play.call(argv: [spec, "--db", db], stdout: out)
  ...
  def test_pwd_output_in_body_spec_passes_under_play_cli
    repo_root = File.expand_path("../../..", __dir__)
    Dir.chdir(repo_root) do
      spec = File.join(repo_root, "baslash-debug/test/specs/pwd_output_in_body.rb")
      out  = StringIO.new
  ...
  def test_no_alt_screen_spec_passes_under_play_cli
    repo_root = File.expand_path("../../..", __dir__)
    Dir.chdir(repo_root) do
      spec = File.join(repo_root, "baslash-debug/test/specs/no_alt_screen.rb")
      out  = StringIO.new
  ...
  def test_layout_after_slash_spec_passes_under_play_cli
    repo_root = File.expand_path("../../..", __dir__)
    Dir.chdir(repo_root) do
      spec = File.join(repo_root, "baslash-debug/test/specs/layout_after_slash.rb")
      out  = StringIO.new
  ...
  def test_winsize_stale_env_spec_passes_under_play_cli
    repo_root = File.expand_path("../../..", __dir__)
    Dir.chdir(repo_root) do
      spec = File.join(repo_root, "baslash-debug/test/specs/winsize_stale_env.rb")
      out  = StringIO.new
