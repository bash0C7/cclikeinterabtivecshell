...
`baslash-debug` can record any TUI binary running under a PTY, drive it
from a Ruby DSL spec, and replay or hex-dump the result. The recording
lives in the same SQLite store as baslash framework sessions, in two
...
| Command                                | Purpose                                            |
| -------------------------------------- | -------------------------------------------------- |
| `baslash-debug play <spec.rb>`        | Run a DSL spec, persist + run `expect` blocks.     |
| `baslash-debug replay <session_uuid>` | Stream recorded output to stdout with real timing. |
| `baslash-debug dump <session_uuid>`   | Emit hex/ascii per event for grep-style inspection.|
| `baslash-debug pty-list`              | List recorded sessions newest-first.               |

All four accept `--db PATH` (default `tmp/longrun/baslash-debug.sqlite`
under the current working directory, falling back to `./baslash-debug.sqlite`).
