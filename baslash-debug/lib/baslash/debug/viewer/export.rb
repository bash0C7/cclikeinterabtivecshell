...
module Baslash
  module Debug
    module Viewer
      module Export
        def self.call(argv)
          session = argv.shift or abort("usage: baslash-debug export <session> --format=cast|gif|mp4|webm [--output PATH]")
          ...
          when "cast"
            File.open(output, "w") do |f|
              Baslash::Debug::CastWriter.write(f, frames: frames, rows: info[:rows], cols: info[:cols], started_at: 0)
            end
            puts output
          when "gif"
            cast = "/tmp/baslash-export-#{Process.pid}.cast"
          ...
          when "mp4", "webm"
            cast = "/tmp/baslash-export-#{Process.pid}.cast"
            gif  = "/tmp/baslash-export-#{Process.pid}.gif"
            File.open(cast, "w") do |f|
              Baslash::Debug::CastWriter.write(f, frames: frames, rows: info[:rows], cols: info[:cols], started_at: 0)
            end
