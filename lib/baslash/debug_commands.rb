  ...
  module DebugCommands
    ...
    def self.register_sleep(registry)
      registry.register(:"debug-sleep", ->(args, ctx) {
        n = Baslash::DebugCommands.parse_sleep_arg(args.first)
        if n.nil?
    ...
    def self.register_emit(registry)
      registry.register(:"debug-emit", ->(args, ctx) {
        ...
        joined = args.join(" ")
        begin
          bytes = Baslash::DebugCommands::EscapeInterpreter.parse(joined)
    ...
    def self.register_frame_dump(registry)
      registry.register(:"debug-frame-dump", ->(_args, ctx) {
        unless defined?(Baslash::DebugEndpoint) && Baslash::DebugEndpoint.respond_to?(:latest_frame_bytes)
          ctx.display.append("baslash-debug endpoint not enabled", style: :error)
          next
        end
        bytes = Baslash::DebugEndpoint.latest_frame_bytes
      ...
      }, description: "hex-dump the latest baslash-debug captured frame")
    end
