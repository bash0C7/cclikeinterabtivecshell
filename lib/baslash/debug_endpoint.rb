...
module Baslash
  module DebugEndpoint
    ...
    # baslash-debug session DB, or nil when:
    #   - endpoint isn't holding a DB handle (@db unset)
    #   - DB has no non-NULL raw_bytes_zlib rows
    #   - decompress fails
    # Errors are logged via Baslash::Context.logger and swallowed (returning nil),
    ...
    def self.latest_frame_bytes
      ...
      Baslash::Context.logger.error("DebugEndpoint.latest_frame_bytes failed: #{e.class}: #{e.message}") rescue nil
      nil
    end
    ...
    class Adapter
      ...
      def build_framework_state_hash
        ...
        {
          phase:             Baslash::Context.state[:phase],
          focus_mode:        Baslash::Context.state[:focus_mode],
          ...
          popup:             popup_state,
          transcript_line_count: Baslash::Transcript.lines.size
        }
      end
      ...
      def live_slot_state
        ...
        slots = (Baslash::Display.respond_to?(:live_slot_state) ? Baslash::Display.live_slot_state : {}) rescue {}
